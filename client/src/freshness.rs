...
impl NonceManager {
    ...
    pub fn validate_nonce_response(&mut self, nonce: &[u8]) -> Result<()> {
        ...
        let entry = self.pending_nonces.remove(nonce).ok_or_else(|| {
        ...
        if age > self.max_nonce_age {
            return Err(ClientError::Client(
                crate::EphemeralError::AttestationError(format!(
                    "Nonce expired: age {}s exceeds limit {}s",
                    age, self.max_nonce_age
                )),
            ));
        }
    ...
    pub fn cleanup_expired(&mut self) {
        ...
        self.pending_nonces
            .retain(|_, entry| current_time.saturating_sub(entry.timestamp) < self.max_nonce_age);
...
impl FreshnessValidator {
    ...
    pub fn validate_timestamp(&self, timestamp: u64) -> Result<()> {
        ...
        if timestamp > current_time + self.max_time_skew {
            return Err(ClientError::Client(
                crate::EphemeralError::AttestationError(format!(
                    "Timestamp too far in future: {} vs current {}",
        ...
        if age > self.min_freshness {
            return Err(ClientError::Client(
                crate::EphemeralError::AttestationError(format!(
                    "Timestamp too old: age {}s exceeds freshness limit {}s",
                    age, self.min_freshness
                )),
            ));
        }
...
mod tests {
    ...
    fn test_freshness_enforcer_integration() {
        ...
        assert!(enforcer
            .validate_attestation_response(&nonce2, timestamp)
            .is_err());
    }
