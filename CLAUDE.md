...
### Attested GenerateDataKey

As of commit `c6aa5b3`, `KmsRequest::GenerateDataKey` supports `encryption_context` and `recipient` (attestation document) fields, matching the `Decrypt` variant's pattern. When `recipient` is provided, KMS returns the DEK wrapped for the enclave's RSA public key via `ciphertext_for_recipient` instead of plaintext — the host never sees the key material.

`KmsResponse::GenerateDataKey.plaintext` is now `Option<Vec<u8>>` (was `Vec<u8>`). When a recipient is provided, `plaintext` is `None` and `ciphertext_for_recipient` contains the wrapped key. Both the production path (`kms_proxy_server.rs`) and mock path process attestation documents for GenerateDataKey, with fail-closed checks mirroring the Decrypt path.

The enclave benchmark (`vsock-pingpong --mode benchmark`) was simplified from a two-step flow (GenerateDataKey → Decrypt with RecipientInfo) to a single attested GenerateDataKey call. This correctly measures `kms_key_release_ms` as one KMS round-trip instead of two. The diagnostic mode (`--mode kms`) retains the two-step flow for protocol coverage.
