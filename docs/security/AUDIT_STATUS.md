...
- **Closed in the current lockfile (2026-09-19):** the affected `rand 0.8.5` / `rand 0.9.2` versions from RUSTSEC-2026-0097 are no longer present (`rand 0.8.6` / `rand 0.9.3` are resolved).
- **Informational / upstream warnings:** `paste 1.0.15` and `serde_cbor 0.11.2` are unmaintained transitive dependencies. Yanked `spin 0.9.8` and `spin 0.10.0` remain through `multer` and `crc-fast`; no vulnerability advisory is attached to either warning. All four are tracked in the risk register.
...
`cargo audit` scanned the 579-package lockfile on 2026-09-19 and reported zero
known vulnerabilities plus the four informational/upstream warnings above.
