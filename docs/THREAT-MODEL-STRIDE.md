...
|---|---|---|---|---|---|
| T-01 | A-01 input | **S**poofing | Attacker impersonates a legitimate tenant, sends request as if from another | A non-loopback gateway fails closed unless a shared bearer token (`EPHEMERALML_API_KEY`) is configured or the operator sets the explicit development-only insecure override. Tokens are compared in constant time. This is **not** per-tenant auth — all holders remain indistinguishable to the gateway. | **partial** — shared-token control; per-tenant identity remains deployment-dependent. See RR-11. |
...
| RR-11 | Gateway auth is a shared bearer token | `EPHEMERALML_API_KEY` is required for non-loopback listeners unless an explicit insecure development override is set; comparison is constant-time. All token holders are still indistinguishable. | Upstream auth proxy per deployment; token rotation policy per pilot. Per-tenant identity at the gateway layer is a product roadmap item, not a current control. Do not claim multi-tenant isolation at this layer without it. |
| RR-12 | A remote OpenAI-compatible gateway is a plaintext trust boundary | The gateway parses prompts before constructing the attested SecureChannel request. Local-proxy defaults bind to loopback, require preflight, and non-loopback unauthenticated binds fail closed. | Run `cyntrisec-proxy` on the customer side for the strongest boundary; use TLS and a trusted authorization proxy if intentionally deploying the HTTP gateway remotely. Do not claim that a remote plaintext-terminating gateway is invisible to its host operator. |
