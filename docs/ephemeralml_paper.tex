...
% --- Title ---
% --- Paper benchmark macros (update after AWS reruns) ---
\newcommand{\BenchInstance}{m6i.xlarge}
\newcommand{\BenchCommit}{b00bab1}
\newcommand{\BenchModelName}{MiniLM-L6-v2}
\newcommand{\BenchModelParams}{22.7M}

\newcommand{\BenchBareMeanMs}{78.55}
\newcommand{\BenchEnclaveMeanMs}{88.45}
\newcommand{\BenchOverheadPct}{12.6\%}
\newcommand{\BenchBareP95Ms}{79.09}
\newcommand{\BenchEnclaveP95Ms}{89.58}
\newcommand{\BenchBareThroughput}{12.73}
\newcommand{\BenchEnclaveThroughput}{11.31}
\newcommand{\BenchBarePeakRssMb}{266}
\newcommand{\BenchEnclavePeakRssMb}{1018}

\newcommand{\BenchAttestationMs}{88}
\newcommand{\BenchKmsMs}{76}
\newcommand{\BenchS3FetchMs}{6716}
\newcommand{\BenchDecryptMs}{100}
\newcommand{\BenchLoadMs}{39}
\newcommand{\BenchTokenizerMs}{24}
\newcommand{\BenchColdStartTotalMs}{7052}

% Multi-model scaling table (fill after multi-model AWS run completes)
\newcommand{\ModelL12BareMeanMs}{TBD}
\newcommand{\ModelL12EnclaveMeanMs}{TBD}
\newcommand{\ModelL12OverheadPct}{TBD}
\newcommand{\ModelBertBaseBareMeanMs}{TBD}
\newcommand{\ModelBertBaseEnclaveMeanMs}{TBD}
\newcommand{\ModelBertBaseOverheadPct}{TBD}

\title{\textbf{EphemeralML: Confidential AI Inference on AWS Nitro Enclaves\\with \BenchOverheadPct{} Overhead}}
\author{Cyntrisec}
\date{February 2026}
...
We evaluate \textsc{EphemeralML} on an \texttt{\BenchInstance{}} instance using \BenchModelName{} (\BenchModelParams{} parameters) and measure \textbf{\BenchOverheadPct{} mean inference overhead} (\BenchBareMeanMs{}\,ms bare metal vs.\ \BenchEnclaveMeanMs{}\,ms enclave), near-identical embedding output (cosine similarity $\approx 1.0$), a per-inference cryptographic budget of 0.028\,ms for 1\,KB payloads (enclave-side), and an end-to-end per-request crypto overhead of 0.164\,ms excluding inference.
...
\begin{enumerate}
  ...
  \item A \textbf{comprehensive evaluation} showing \BenchOverheadPct{} inference overhead, sub-millisecond per-request cryptographic cost, and near-identical output quality on \BenchModelName{}.
\end{enumerate}
...
\begin{itemize}
  \item \textbf{KmsProxy}: Forwards attestation-gated KMS \texttt{Decrypt} requests.
...
On startup, it:
(1) generates an attestation document via the Nitro Security Module (NSM),
(2) generates ephemeral X25519 and Ed25519 key pairs and embeds the public keys in the attestation user data,
(3) obtains the model decryption key from KMS (attestation-gated),
(4) fetches and decrypts model artifacts via the host's S3 proxy,
(5) loads the model into the Candle inference engine, and
(6) serves encrypted inference requests over VSock.
...
\begin{itemize}
  ...
  \item Model ID, version, execution time, and peak memory
  \item Monotonic sequence number
\end{itemize}

The receipt is CBOR-encoded with deterministic sorted map keys, then signed with the enclave's ephemeral Ed25519 private key (64-byte signature).
...
\textsc{EphemeralML} is implemented in 13,000+ lines of Rust (2021 edition, stable toolchain, MSRV 1.75+) organized as a Cargo workspace with four crates:
...
\begin{itemize}
  ...
  \item \textbf{\texttt{production}}: Uses real NSM API (\texttt{aws-nitro-enclaves-nsm-api}~0.4) and \texttt{tokio-vsock}~0.4.
\end{itemize}

\paragraph{Inference Engine.}
Model inference uses the Candle framework (\texttt{candle-core}~0.9, \texttt{candle-nn}~0.9) with SafeTensors~0.7 for weight deserialization and HuggingFace Tokenizers~0.21.
...
All tests pass in CI (GitHub Actions) on every push to \texttt{main}.
...
The benchmark model is \BenchModelName{} (\BenchModelParams{} parameters, 384-dimensional embeddings).
All latency measurements use 100 iterations after 3 warmup runs.
Results are highly reproducible: in two steady-state runs (runs 2--3 at commit \texttt{\BenchCommit{}}), inference overhead ranged from 12.4--12.8\%; the first run after a clean build was a cold-cache outlier at 17.6\%.
...
\begin{table}[ht]
\centering
\caption{Inference latency comparison (\BenchModelName{}, \BenchInstance{}, 100 iterations).}
\label{tab:inference}
\begin{tabular}{lrrr}
\hline
...
P95 latency      & \BenchBareP95Ms{}\,ms  & \BenchEnclaveP95Ms{}\,ms  & --- \\
Throughput       & \BenchBareThroughput{}\,inf/s & \BenchEnclaveThroughput{}\,inf/s & --- \\
Peak RSS (VmHWM) & \BenchBarePeakRssMb{}\,MB    & \BenchEnclavePeakRssMb{}\,MB & --- \\
\hline
\end{tabular}
\end{table}

\noindent\textbf{Note:} Peak RSS is obtained from \texttt{/proc/self/status} (VmHWM). Earlier benchmark code used VmPeak (virtual memory size); regenerate benchmark artifacts to ensure this row reflects resident memory.

The \BenchOverheadPct{} mean inference overhead falls within the ``Acceptable'' range and is consistent with published AMD SEV-SNP results for BERT-class models~\cite{akram2024}.

\subsection{Overhead vs.\ Model Size (Encoder Models)}

Table~\ref{tab:model-scaling} summarizes overhead across multiple encoder-style models.
Fill in the MiniLM-L12 and BERT-base rows after the multi-model AWS rerun completes.

\begin{table}[ht]
\centering
\caption{Overhead scaling across encoder models (\BenchInstance{}, 100 iterations).}
\label{tab:model-scaling}
\begin{tabular}{lrrrr}
\hline
\textbf{Model} & \textbf{Params} & \textbf{Bare Metal (mean)} & \textbf{Enclave (mean)} & \textbf{Overhead} \\
\hline
\BenchModelName{} & \BenchModelParams{} & \BenchBareMeanMs{}\,ms & \BenchEnclaveMeanMs{}\,ms & \BenchOverheadPct{} \\
MiniLM-L12-v2 & 33.4M & \ModelL12BareMeanMs{} & \ModelL12EnclaveMeanMs{} & \ModelL12OverheadPct{} \\
BERT-base-uncased & 110M & \ModelBertBaseBareMeanMs{} & \ModelBertBaseEnclaveMeanMs{} & \ModelBertBaseOverheadPct{} \\
\hline
\end{tabular}
\end{table}
...
S3 model fetch (via VSock) & \BenchS3FetchMs{}\,ms \\
...
Cold start is dominated by the S3 model fetch over VSock (92.5\% of total).
This could be reduced by embedding weights in the EIF image or implementing a pre-warming protocol.
The one-time attestation and KMS costs (\BenchAttestationMs{} + \BenchKmsMs{} $\approx$ 164\,ms combined) are negligible for session-based workloads.
...
1\,MB   & 4.25\,ms \\
\hline
\multicolumn{2}{l}{Upload throughput: 235.4\,MB/s} \\
\hline
\end{tabular}
\end{table}
...
X25519 keypair generation       & 0.017\,ms & 0.025\,ms \\
HPKE encrypt 1\,KB              & 0.003\,ms & 0.003\,ms \\
HPKE decrypt 1\,KB              & 0.003\,ms & 0.003\,ms \\
HPKE encrypt 1\,MB              & 0.92\,ms  & 0.95\,ms \\
HPKE decrypt 1\,MB              & 0.97\,ms  & 1.00\,ms \\
Ed25519 keypair generation      & 0.017\,ms & 0.019\,ms \\
Receipt sign (CBOR + Ed25519)   & 0.022\,ms & 0.023\,ms \\
Receipt verify                  & 0.046\,ms & 0.054\,ms \\
\hline
...
The per-inference cryptographic budget of 0.028\,ms represents $\sim$0.03\% of the 79\,ms inference latency, confirming that encryption overhead is negligible.
...
\textbf{Operation} & \textbf{Mean} & \textbf{P99} \\
\hline
COSE\_Sign1 verify (ECDSA-P384) & 0.737\,ms & 0.762\,ms \\
Certificate chain walk (3 certs) & 2.224\,ms & 2.259\,ms \\
CBOR payload parse               & 0.001\,ms & 0.002\,ms \\
\hline
\textbf{Full verification pipeline} & \textbf{2.998\,ms} & \textbf{3.038\,ms} \\
\hline
\end{tabular}
\end{table}
...
\hline
Per-request crypto (encrypt + decrypt + receipt) & 0.164\,ms \\
Session setup (one-time)                         & 0.138\,ms \\
TCP handshake (one-time)                         & 0.153\,ms \\
\hline
\end{tabular}
\end{table}
...
\hline
1 & 12.75\,inf/s & 78.4\,ms  & 100\% \\
2 & 14.73\,inf/s & 135.8\,ms & 57.8\% \\
4 & 14.66\,inf/s & 270.4\,ms & 28.8\% \\
8 & 14.57\,inf/s & 546.1\,ms & 14.3\% \\
\hline
\end{tabular}
\end{table}

Throughput plateaus at 2 threads ($\sim$14.3\,inf/s), indicating that MiniLM inference on this instance is compute-bound on the available 4 vCPUs.
...
We compare the embedding vectors produced by bare-metal and enclave inference for identical inputs.
Across repeated runs in each environment, the reference embedding SHA-256 is stable (bit-identical), demonstrating deterministic output under fixed inputs and software versions.
Cross-environment comparisons show near-identical embeddings (cosine similarity $\approx 1.0$ with small floating-point differences), indicating negligible numerical divergence for CPU inference in the enclave.

\subsection{Cost Analysis}

Table~\ref{tab:cost} shows the per-inference cost on \texttt{m6i.xlarge} (\$0.192/hr on-demand).

\begin{table}[ht]
\centering
\caption{Cost analysis (m6i.xlarge, \$0.192/hr on-demand pricing).}
\label{tab:cost}
\begin{tabular}{lrr}
\hline
...
The 15\% cost premium for confidential inference is modest, especially given that the enclave provides hardware-enforced isolation, cryptographic attestation, and end-to-end encryption.
...
AMD SEV-SNP provides VM-level isolation with lower overhead; published BERT inference benchmarks report 10--15\% overhead~\cite{akram2024}, consistent with our 14.5\% result on Nitro Enclaves for a comparably sized model (MiniLM-L6-v2, 22.7M parameters).
However, overhead scales with model size: Schnabl et~al.~\cite{schnabl2025} report $2.2\times$ CPU overhead for Llama-3.1-8B (8 billion parameters) running inside Nitro Enclaves, driven by increased memory pressure and enclave kernel overhead at scale.
This suggests that our \BenchOverheadPct{} result, while representative for embedding-class models, should not be extrapolated to billion-parameter LLMs without further evaluation.
Intel TDX offers similar VM-level confidential computing; early benchmarks suggest comparable overhead for inference workloads~\cite{intel2023}.

\paragraph{GPU TEEs.}
NVIDIA's Confidential Computing (cGPU) with H100 GPUs enables TEE-protected GPU inference.
Early measurements show 5--8\% overhead for large language models~\cite{nvidia2024}, though the threat model differs (GPU memory is trusted, CPU-GPU transfers are encrypted).
...
Lutsch et~al.~\cite{lutsch2025} analyze Nitro Enclave overhead for analytical database workloads, characterizing the VSock communication bottleneck that also dominates \textsc{EphemeralML}'s cold start.
...
Fully homomorphic encryption (FHE) and secure multi-party computation (MPC) can provide inference without trusting any party, but current implementations incur $10{,}000\times$+ overhead for neural network inference~\cite{gilad2016}, making them impractical for latency-sensitive workloads.
...
The \BenchColdStartTotalMs{}\,ms cold start is dominated by fetching the model over VSock from S3 (\BenchS3FetchMs{}\,ms).
...
The memory overhead (266\,MB vs.\ 1{,}018\,MB peak RSS in our steady-state runs) is attributable to the enclave kernel, VSock message buffers, and additional runtime footprint inside the enclave.
Optimizing the enclave image (e.g., using a minimal init process, pre-allocating buffers) could reduce this.
...
Throughput saturates at 2 threads on 4 vCPUs due to compute-bound GEMM operations.
...
Our \BenchOverheadPct{} overhead result is measured on \BenchModelName{} (\BenchModelParams{} parameters).
Published results for Llama-3.1-8B on Nitro Enclaves show $2.2\times$ overhead~\cite{schnabl2025}, indicating that enclave overhead grows substantially with model size due to increased memory pressure, larger VSock transfers, and enclave kernel overhead.
Evaluating \textsc{EphemeralML} on billion-parameter models---potentially with INT4/INT8 quantization to fit within enclave memory constraints---is an important direction for validating the system's applicability to large language models.
...
Our evaluation on MiniLM-L6-v2 demonstrates that hardware-enforced confidential inference is practical:
\BenchOverheadPct{} inference overhead, 0.028\,ms per-inference (enclave-side) cryptographic cost, near-identical output quality, and a $\sim$13\% cost premium over bare-metal execution.
...
\textsc{EphemeralML} is open-source and available at \url{https://github.com/cyntrisec/EphemeralML}.

% ============================================================
\begin{thebibliography}{10}

\bibitem{akram2024}
A.~Akram, A.~Mukherjee, and M.~Huzaifa,
``Performance analysis of confidential VMs for deep learning inference,''
in \emph{Proc.\ IEEE International Symposium on Secure and Private Execution Environment Design (SEED)}, 2024.
