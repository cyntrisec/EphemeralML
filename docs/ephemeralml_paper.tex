...
% --- Claim language cross-reference ---
% All performance claims in this paper use their own provenance (commit 057a85a,
% canonical release-gate run). For the unified claim registry covering ALL
% public-facing numbers (including this paper), see:
%   docs/publication/claim_definitions.md
% The paper's internal overhead numbers (e.g. \BenchOverheadPct) are NOT
% directly edited here — they have independent measurement provenance.
% External headline: "+3--13\% depending on measurement boundary" (§3 of
% claim_definitions.md).
...
% MiniLM-L12 & BERT-base: 3 runs from artifacts/benchmarks/multimodel-20260205
...
% Multi-model scaling table (3-run means from artifacts/benchmarks/multimodel-20260205)
...
\textsc{EphemeralML} is implemented in 13,000+ lines of Rust (2021 edition, stable toolchain, MSRV 1.95+) organized as a Cargo workspace with four core crates (plus a VSock benchmarking utility):
...
\paragraph{Future Hardening.}
The current v1 implements the Gateway path only. Future work may add query budgets, differential privacy noise injection, and rate limiting at the enclave level, but these controls are out of scope for the present system and receipt format.
