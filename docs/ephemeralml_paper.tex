...
% --- Title ---
% --- Paper benchmark macros (3-run averages, commit 8fc6c36, 2026-02-05) ---
\newcommand{\BenchInstance}{m6i.xlarge}
\newcommand{\BenchCommit}{8fc6c36}
\newcommand{\BenchModelName}{MiniLM-L6-v2}
\newcommand{\BenchModelParams}{22.7M}
\newcommand{\BenchNumRuns}{3}

\newcommand{\BenchBareMeanMs}{79.28}
\newcommand{\BenchEnclaveMeanMs}{90.36}
\newcommand{\BenchOverheadPct}{14.0\%}
\newcommand{\BenchBarePNinetyFiveMs}{81.07}
\newcommand{\BenchEnclavePNinetyFiveMs}{92.44}
\newcommand{\BenchBareThroughput}{12.61}
\newcommand{\BenchEnclaveThroughput}{11.07}
\newcommand{\BenchBarePeakRssMb}{266}
\newcommand{\BenchEnclavePeakRssMb}{586}

\newcommand{\BenchAttestationMs}{208}
\newcommand{\BenchKmsMs}{82}
\newcommand{\BenchSThreeFetchMs}{5908}
\newcommand{\BenchDecryptMs}{101}
\newcommand{\BenchLoadMs}{40}
\newcommand{\BenchTokenizerMs}{24}
\newcommand{\BenchColdStartTotalMs}{6372}
...
\newcommand{\BenchOverheadRange}{12--14\%}
...
All latency measurements use 100 iterations after 3 warmup runs, repeated \BenchNumRuns{} times per model at commit \texttt{\BenchCommit{}}.
...
P95 latency      & \BenchBarePNinetyFiveMs{}\,ms  & \BenchEnclavePNinetyFiveMs{}\,ms  & +14.0\% \\
Throughput       & \BenchBareThroughput{}\,inf/s & \BenchEnclaveThroughput{}\,inf/s & $-$12.2\% \\
Peak RSS (VmHWM) & \BenchBarePeakRssMb{}\,MB    & \BenchEnclavePeakRssMb{}\,MB & +120\% \\
\hline
\end{tabular}
\end{table}

The \BenchOverheadPct{} mean inference overhead for \BenchModelName{} falls within the ``Acceptable'' range and is consistent with published AMD SEV-SNP results for BERT-class models~\cite{akram2024}.
...
Notably, BERT-base (110M parameters) has a lower overhead percentage (+\ModelBertBaseOverheadPct{}) than \BenchModelName{} (+\BenchOverheadPct{}) despite being 5$\times$ larger, suggesting that enclave overhead is partly fixed-cost (kernel, VSock buffers) rather than proportional to model size.
...
The one-time attestation and KMS costs (\BenchAttestationMs{} + \BenchKmsMs{} $\approx$ 290\,ms combined) are negligible for session-based workloads.
...
\hline
\textbf{Full verification pipeline} & \textbf{2.999\,ms} & \textbf{3.069\,ms} \\
\hline
\end{tabular}
\end{table}
...
The linear fit is $\mathrm{latency} = -18.1\,\text{ms} + 0.953\,\text{ms/token}$, confirming that BERT self-attention complexity dominates and scales linearly with sequence length in this range.
...
\hline
\multicolumn{5}{l}{\textbf{Crypto overhead per request: 0.301\,ms} (0.38\% of E2E)} \\
\hline
\end{tabular}
\end{table}

The 0.301\,ms crypto overhead per request confirms that HPKE encryption, decryption, and receipt signing/verification add negligible latency compared to model inference.
...
\hline
Inferences/hour        & 45{,}393 & 39{,}858 \\
Cost per 1M inferences & \$4.23 & \$4.82 \\
Cost multiplier        & ---    & 1.14$\times$ \\
\hline
\end{tabular}
\end{table}

The 14\% cost premium for confidential inference is modest, especially given that the enclave provides hardware-enforced isolation, cryptographic attestation, and end-to-end encryption.
...
AMD SEV-SNP provides VM-level isolation with lower overhead; published BERT inference benchmarks report 10--15\% overhead~\cite{akram2024}, consistent with our \BenchOverheadRange{} result on Nitro Enclaves across three encoder models (22.7M--110M parameters).
...
However, published results for Llama-3.1-8B on Nitro Enclaves show $2.2\times$ overhead~\cite{schnabl2025}, indicating that this amortization effect does not hold at the billion-parameter scale, where memory pressure, page faults, and enclave kernel overhead become dominant.
...
\BenchOverheadRange{} mean inference overhead (CV $< 1\%$ across runs), 0.028\,ms per-inference cryptographic cost, bit-identical output within environments, near-identical output across environments (cosine similarity $\approx 1.0$), and a $\sim$14\% cost premium over bare-metal execution.
