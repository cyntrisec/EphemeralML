...
% --- Title ---
\title{\textbf{EphemeralML: Confidential AI Inference on AWS Nitro Enclaves\\with 14.5\% Overhead}}
\author{Cyntrisec}
\date{February 2026}
...
The host operates as a \emph{blind relay}: it forwards encrypted bytes over VSock but can never decrypt or inspect sensitive data, even under full root compromise.
...
We evaluate \textsc{EphemeralML} on an \texttt{m6i.xlarge} instance using MiniLM-L6-v2 (22.7M parameters) and measure \textbf{14.5\% mean inference overhead} (81.32\,ms bare metal vs.\ 93.08\,ms enclave), near-identical embedding output (cosine similarity $\approx 1.0$), a per-inference cryptographic budget of 0.027\,ms for 1\,KB payloads (enclave-side), and an end-to-end per-request crypto overhead of 0.162\,ms excluding inference.
The system is implemented in 13,000+ lines of Rust across four workspace crates, compiles for both local mock and production Nitro modes via feature flags, and is fully open-source.
\end{abstract}
...
\begin{enumerate}
  ...
  \item A \textbf{comprehensive evaluation} showing 14.5\% inference overhead, sub-millisecond per-request cryptographic cost, and near-identical output quality on MiniLM-L6-v2.
\end{enumerate}
...
\begin{enumerate}
  \item Parse the COSE\_Sign1 structure and extract the certificate chain (COSE label~33, \texttt{x5chain}).
...
Constant-time comparisons are used for authentication tags and signatures.
...
\paragraph{Testing.}
The test suite comprises 106 tests covering unit tests (inline \texttt{\#[cfg(test)]} modules), integration tests (\texttt{tests/} directories), and end-to-end protocol tests.
...
We evaluate \textsc{EphemeralML} on an \texttt{m6i.xlarge} EC2 instance (4~vCPUs, 16\,GB RAM, Intel Xeon Ice Lake) running Amazon Linux~2023.
The enclave is allocated 2~vCPUs and 4096\,MB RAM.
The benchmark model is MiniLM-L6-v2 (22.7M parameters, 384-dimensional embeddings).
All latency measurements use 100 iterations after 3 warmup runs.
Results are highly reproducible: inference variance $<$1\% across four independent runs.
...
\begin{table}[ht]
\centering
\caption{Inference latency comparison (MiniLM-L6-v2, m6i.xlarge, 100 iterations).}
\label{tab:inference}
\begin{tabular}{lrrr}
\hline
\textbf{Metric} & \textbf{Bare Metal} & \textbf{Enclave} & \textbf{Overhead} \\
\hline
Mean latency     & 81.32\,ms  & 93.08\,ms  & +14.5\% \\
P95 latency      & 83.12\,ms  & 94.95\,ms  & +14.2\% \\
...
The 14.5\% mean inference overhead falls within the ``Acceptable'' range and is consistent with published AMD SEV-SNP results for BERT-class models~\cite{akram2024}.
...
\hline
NSM attestation generation & 277\,ms \\
KMS key release            & 79\,ms \\
S3 model fetch (via VSock) & 6{,}600\,ms \\
Model decryption           & 101\,ms \\
Model load (Candle)        & 40\,ms \\
Tokenizer setup            & 25\,ms \\
\hline
\textbf{Total}             & \textbf{7{,}132\,ms} \\
\hline
\end{tabular}
\end{table}
...
The one-time attestation and KMS costs (356\,ms combined) are negligible for session-based workloads.
...
1\,MB   & 4.56\,ms \\
\hline
\multicolumn{2}{l}{Upload throughput: 219.4\,MB/s} \\
\hline
\end{tabular}
\end{table}
...
\hline
\textbf{Per-inference crypto (1\,KB)} & \textbf{0.027\,ms} & --- \\
\hline
\end{tabular}
\end{table}

The per-inference cryptographic budget of 0.027\,ms represents 0.03\% of the 81\,ms inference latency, confirming that encryption overhead is negligible.
...
\hline
Per-request crypto (encrypt + decrypt + receipt) & 0.162\,ms \\
Session setup (one-time)                         & 0.137\,ms \\
TCP handshake (one-time)                         & 0.176\,ms \\
\hline
\end{tabular}
\end{table}
...
\hline
1 & 12.43\,inf/s & 80.4\,ms  & 100\% \\
2 & 14.32\,inf/s & 139.7\,ms & 57.6\% \\
4 & 14.23\,inf/s & 277.0\,ms & 28.6\% \\
8 & 14.20\,inf/s & 558.7\,ms & 14.3\% \\
\hline
\end{tabular}
\end{table}
...
The cosine similarity is \textbf{0.999999999999926} on the logged reference embedding slice (first 8 dimensions), with maximum absolute difference $3.24\\times 10^{-7}$, indicating negligible numerical divergence for CPU inference in the enclave.
This confirms that TEE execution introduces no numerical divergence for CPU inference.
...
This suggests that our 14.5\% result, while representative for embedding-class models, should not be extrapolated to billion-parameter LLMs without further evaluation.
...
The 7.1-second cold start is dominated by fetching the model over VSock from S3 (6.6\,s).
...
The 2$\times$ memory overhead (535\,MB vs.\ 1{,}064\,MB) is attributable to the enclave kernel, VSock message buffers, and duplicate runtime libraries.
...
Our 14.5\% overhead result is measured on MiniLM-L6-v2 (22.7M parameters, 535\,MB RSS).
...
14.5\% inference overhead, 0.027\,ms per-inference (enclave-side) cryptographic cost, near-identical output quality, and a 15\% cost premium over bare-metal execution.
...
The implementation in 13,000+ lines of Rust, with feature-gated dual compilation and 106 passing tests, demonstrates that confidential ML inference can be achieved with production-quality engineering.
