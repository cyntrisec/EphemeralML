...
The \BenchOverheadPct{} mean inference overhead for \BenchModelName{} falls within the ``Acceptable'' range and is consistent with published AMD SEV-SNP overhead for TensorFlow BERT (2--16\%)~\cite{misono2024}.
...
AMD SEV-SNP provides VM-level isolation with lower overhead; Misono et~al.~\cite{misono2024} report 2--16\% overhead for TensorFlow BERT inference on SEV-SNP (varying with VM configuration), consistent with our \BenchOverheadRange{} result on Nitro Enclaves across three encoder models (22.7M--110M parameters).
However, overhead scales with model size: Schnabl et~al.~\cite{schnabl2025} report $2.2\times$ cost overhead for Llama-3.1-8B-Instruct (8 billion parameters, 4-bit quantized) running inside Nitro Enclaves, driven by the need for larger instances to accommodate enclave memory reservation.
...
Zhu et~al.~\cite{zhu2024} benchmark H100 confidential computing and report 5--7\% throughput overhead for Llama-3.1-8B and Phi-3-14B, though the threat model differs (GPU memory is trusted, CPU-GPU transfers are encrypted).
...
However, published results for Llama-3.1-8B on Nitro Enclaves show $2.2\times$ cost overhead~\cite{schnabl2025}, driven by the need for larger instances to accommodate enclave memory reservation, indicating that the total cost of confidential inference grows substantially at the billion-parameter scale.
...
\bibitem{lee2019}
T.~Lee, Z.~Lin, S.~Pushp, C.~Li, Y.~Liu, Y.~Lee, F.~Xu, C.~Xu, J.~Song, and L.~Zhang,
...
in \emph{ICML Workshop on Technical AI Governance (TAIG)}, 2025.
