...
fn test_spy_proxy_intercepts_and_logs() {
    ...
    let _ = spy_proxy
        .forward_to_enclave(&secret_payload)
    ...
    file.read_to_string(&mut log_content)
        .expect("Failed to read log file");

    // Assert it contains the HEX representation
    let hex_payload: String = secret_payload
        .iter()
        .map(|b| format!("{:02x}", b))
        .collect();
    assert!(
        log_content.contains(&hex_payload),
        "Log should contain hex payload"
    );

    // Assert it DOES NOT contain the plaintext "secret" string
    assert!(
        !log_content.contains("secret"),
        "Log should not contain 'secret'"
    );
    ...
    assert!(
        ...
        "Log should contain dots for non-printable characters"
    );
