...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_weights_bytes: Vec<u8> = test_weights
        .iter()
    ...
    println!(
        "Stored {} bytes of weights for test_model",
        test_weights_bytes.len()
    );

    // Test weight retrieval
    let retrieved_weights_bytes = proxy.retrieve_weights("test_model")?;
    println!(
        "Retrieved {} bytes of weights for test_model",
        retrieved_weights_bytes.len()
    );
