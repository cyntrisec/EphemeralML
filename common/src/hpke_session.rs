...
impl HPKESession {
    ...
    pub fn establish(&mut self, local_private_key: &[u8; 32]) -> Result<()> {
        ...
        self.session_key =
            Self::derive_session_key(shared_secret.as_bytes(), &self.transcript_hash)?;
    ...
    fn derive_session_key(
        ...
        hkdf.expand(transcript_hash, &mut key).map_err(|e| {
            EphemeralError::EncryptionError(format!("HKDF key derivation failed: {:?}", e))
        })?;
    ...
    pub fn decrypt(&mut self, message: &EncryptedMessage) -> Result<Vec<u8>> {
        if !self.is_established {
            return Err(EphemeralError::DecryptionError(
                "Session not established".to_string(),
            ));
        }
    ...
    fn parse_message_frame(&self, frame: &[u8], expected_sequence: u64) -> Result<Vec<u8>> {
        ...
        if version != self.protocol_version {
            return Err(EphemeralError::DecryptionError(
                "Version mismatch in frame".to_string(),
            ));
        }
        offset += 4;
        ...
        let session_id = String::from_utf8(frame[offset..offset + session_id_len].to_vec())
            .map_err(|_| {
                EphemeralError::DecryptionError("Invalid session ID encoding".to_string())
            })?;

        if session_id != self.session_id {
            return Err(EphemeralError::DecryptionError(
                "Session ID mismatch in frame".to_string(),
            ));
        }
        ...
        if offset + 8 > frame.len() {
            return Err(EphemeralError::DecryptionError(
                "Frame too short for sequence".to_string(),
            ));
        }
        ...
        if sequence != expected_sequence {
            return Err(EphemeralError::DecryptionError(
                "Sequence number mismatch".to_string(),
            ));
        }
        offset += 8;
        ...
        if offset + 4 > frame.len() {
            return Err(EphemeralError::DecryptionError(
                "Frame too short for plaintext length".to_string(),
            ));
        }
    ...
    fn encrypt_frame_with_sequence(&self, frame: &[u8], sequence_number: u64) -> Result<Vec<u8>> {
        ...
        let cipher = ChaCha20Poly1305::new_from_slice(&self.session_key).map_err(|e| {
            EphemeralError::EncryptionError(format!("Failed to create cipher: {}", e))
        })?;
    ...
    fn decrypt_frame(&self, ciphertext: &[u8], sequence_number: u64) -> Result<Vec<u8>> {
        ...
        // Create cipher from session key
        let cipher = ChaCha20Poly1305::new_from_slice(&self.session_key).map_err(|e| {
            EphemeralError::DecryptionError(format!("Failed to create cipher: {}", e))
        })?;
...
impl HPKESessionManager {
    ...
    pub fn create_session(
        ...
        if self.sessions.len() >= self.max_sessions {
            return Err(EphemeralError::ResourceExhausted(
...
mod tests {
    ...
    fn test_chacha20_poly1305_encryption() {
        ...
        .unwrap();
        receiver_session
            .establish(server_secret.as_bytes())
            .unwrap();
    ...
    fn test_session_manager() {
        ...
        manager
            .establish_session(&session_id, client_secret.as_bytes())
            .unwrap();
        ...
        server_manager
            ...
            .unwrap();
        server_manager
            .establish_session(&session_id, server_secret.as_bytes())
            .unwrap();
